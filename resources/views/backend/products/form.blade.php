        ...
        <div class="mx-auto max-w-3xl space-y-6 rounded-xl bg-white p-6 shadow">
            <form action="{{ isset($product) ? route('products.update', $product) : route('products.store') }}"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" class="mt-1 block w-full rounded border-gray-300"
                        rows="4" placeholder="Masukkan deskripsi singkat...">
                        ...
                        <select id="fruit_type_id" name="fruit_type_id"
                            ...
                            @foreach ($fruits as $fruit)
                                <option value="{{ $fruit->id }}"
                                    {{ old('fruit_type_id', $product->fruit_type_id ?? '') == $fruit->id ? 'selected' : '' }}>
                                    {{ $fruit->name }}
                                </option>
                            @endforeach
                        </select>
